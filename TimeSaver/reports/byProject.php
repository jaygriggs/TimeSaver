<?php
// $Id: byProject.php,v 1.9
/**
 * @file
 * byProject report generation file.
 *
 * The byProject report uses the PHPExcel libraries to generate an excel5 or 2007 report.
 */
$totalsArray=array();
$ts=new Timesaver();
if ($whichManager>0) {
...
$obj_php_excel->getActiveSheet()->setTitle(listkeeper_value(variable_get('timesaver_list_id_projects', 0),$listkeeper_id,1));
...
$obj_php_excel->getActiveSheet()->setCellValue('H1', t('Report by Project'));
...
$obj_php_excel->getActiveSheet()->getStyle('H1')->getFont()->setSize(10);
...
while ($A=db_fetch_array($res)) {
  ...
  $sql  ="SELECT * FROM {timesaver_timesheet_entry} WHERE uid=%d AND project_id=%d ";
  $sql .="AND datestamp>=%d AND datestamp<=%d AND ( approved=1 ";
  ...
  $xlscol=4;  //column d
  foreach ($_TIMESAVER_LANG_REPORT_COLUMNS as $dbcol=>$name) {
    if ($colcount>4 && $dbcol!='comment' && $startingRowCount != $rowCounter) { //skip out 0-4
  ...
  //$obj_php_excel->getActiveSheet()->setCellValue('A' . $rowCounter, "TOTALS");
